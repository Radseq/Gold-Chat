...
namespace Server.ClientService
{
    class ClientListManager : ServerResponds, IPrepareRespond
    {
        ...
        public void Execute()
        {
            ...
            {
                SendChannelList sendChannelList = new SendChannelList(ChannelList, Send);
                sendChannelList.Execute();
            }
            else if (Received.strMessage == "ChannelUsers") //send user with enter to channel list of users
            {
                SendListOfUsersInChannel sendListOfUsersInChannel = new SendListOfUsersInChannel(ChannelList, Send, Received);
                sendListOfUsersInChannel.Execute();
            }
            else {
                SendOnlineListOfClients sendOnlineUsers = new SendOnlineListOfClients(ClientList, Send);
